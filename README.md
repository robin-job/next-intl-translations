<h1 align="center">
  ...
  <a href="https://next-intl.dev">
    <picture>
      <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/amannn/next-intl/HEAD/media/logo-dark-mode.svg">
      <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/amannn/next-intl/HEAD/media/logo.svg">
      <img alt="next-intl" src="https://raw.githubusercontent.com/amannn/next-intl/HEAD/media/logo.svg" width="600">
    </picture>
  </a>
  <br>
  <br>
...
2. Configure the `next-intl` in your `next.config.js `, provider and other settings according to [documentation](https://next-intl.dev):
    ```ts
    // next.config.js
...
3. Specify the available languages:
    ...
    ```
4. Specify the type `TLocales` in `global.d.ts` so that it works in the `translations.ts` files, or your version starts with `next-intl@4.0 `, then in the `global.ts` file:
    ```ts
      // global.d.ts
